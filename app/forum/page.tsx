...
const forumTopics: ForumTopic[] = [
  {
    ...
    content: "Bonjour à tous j'essaie de filtrer les éléments aux click selon une propriété sauf qu'il ne filtre p...",
    status: "Résolu"
  },
  {
    id: 2,
    ...
    content: "Je souhaite créer un système avec Laravel où, lors de l'inscription d'un utilisateur, un numéro/matr...",
    status: "Résolu"
  },
  {
    id: 3,
    ...
    content: "Bonjour à tous je viens vers vous parce que je rencontre un problème avec mon token qui n'expire pas...",
    status: "Non résolu"
  },
  {
    id: 4,
    ...
    content: "Salut tout le monde, j'ai un souci avec l'intégration de Tailwind CSS dans mon projet React. Les styles ne s'appliquent pas correctement...",
    status: "Résolu"
},
{
  id: 5,
  ...
  content: "Bonjour à tous, je reçois une erreur 500 lorsque j'essaie d'envoyer un e-mail avec Laravel. J'ai configuré mon `.env` mais ça ne marche toujours pas...",
  status: "Résolu"
},
{
  id: 6,
  ...
  content: "Bonjour, j'utilise Vue.js pour gérer la validation de mes formulaires, mais les messages d'erreur ne s'affichent pas correctement. Quelqu'un a une idée ?",
  status: "Non résolu"
},

{
  ...
  content: "Salut à tous, je rencontre un problème de CORS quand j'essaie de faire une requête API depuis mon application Angular. Une idée pour contourner cela ?",
  status: "Résolu"
}
...
const ForumPage = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState('Toutes les discussions');
  ...
  const filteredTopics = forumTopics.filter(topic => {
    ...
    const matchesCategory = selectedCategory === 'Toutes les discussions' || 
                          topic.categories.includes(selectedCategory);
    const matchesStatus = selectedFilter === 'Tout' || topic.status === selectedFilter;
                        ...
                        {category.subcategories.map((sub) => (
                          <button
                            ...
                            className={`text-sm block w-full text-left py-1 ${
                              selectedCategory === sub ? 'text-primary' : 'text-gray-600'
                            }`}
                          >
                            {sub}
                          </button>
          ...
          <div className="flex-1">
                    ...
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
                          ...
                          <span className="text-gray-400">{topic.authorUsername}</span>
                          <span>•</span>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {topic.categories.map((category) => (
                            <span key={category} className="bg-gray-100 text-sm px-2 py-1 rounded-full">
                              {category}
                            </span>
                          ))}
                          <div className="flex items-center gap-4 ml-auto text-sm text-gray-500">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              topic.status === 'Résolu' ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-700'
                            }`}>
                              {topic.status}
                            </span>
                          </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              <span className="text-sm text-gray-500">
                Affichage de {((currentPage - 1) * ITEMS_PER_PAGE) + 1} à {Math.min(currentPage * ITEMS_PER_PAGE, filteredTopics.length)} sur {filteredTopics.length} résultats
              </span>
                ...
                {pageNumbers.map((page) => (
                  <button
                    ...
                    className={`px-3 py-1 border rounded ${
                      page === currentPage ? 'bg-primary text-white' : 'hover:bg-gray-50'
                    }`}
                  >
                    {page}
                  </button>
