...
function bio(bio: string | null) {
  const charsPerLine = 50;
  const maxChars = charsPerLine * 2.5;
  if (bio && bio.length > maxChars) {
    return bio.slice(0, maxChars - 1) + "…";
  }
  return bio || "Pas de bio!";
}
