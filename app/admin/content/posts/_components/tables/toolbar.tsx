...
import { DRAFT_OPTIONS, useTableFilters } from "./use-table-filter";

// Define the draft filter options

type DraftOption = keyof typeof DRAFT_OPTIONS;
...
export function BlogFiltersCompact({ className }: BlogFiltersProps) {
  ...
  const handleDraftFilterChange = useCallback(
    (value: string) => {
      setDraftFilter(value as DraftOption);
    },
    [setDraftFilter]
  );
  ...
  return (
    <div className={`flex items-center gap-4 ${className}`}>
      ...
      {/* Status Filter */}
      <Select value={draftFilter} onValueChange={handleDraftFilterChange}>
