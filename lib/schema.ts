...
// Blog Reaction Table (Handling reactions for blog posts)
export const blogReaction = pgTable("blog_reaction", {
  id: text("id")
    .primaryKey()
    .$defaultFn(() => crypto.randomUUID()),
...
// Forum Post Relations (fixing the missing vote relation)
export const forumPostRelations = relations(forumPost, ({ one, many }) => ({
  author: one(user, {
    fields: [forumPost.authorId],
    references: [user.id],
  ...
  replies: many(forumReply),
  //votes: many(forumVote), // Added relation for votes
}));

// Forum Reply Relations (fixing missing relation)
export const forumReplyRelations = relations(forumReply, ({ one, many }) => ({
  author: one(user, {
    fields: [forumReply.authorId],
    references: [user.id],
