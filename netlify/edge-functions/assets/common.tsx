...
const FONTS = [
  {
    ...
    weight: 300,
    style: "normal",
    filePath: "Montserrat-Regular.ttf",
  },
  {
    name: "montserrat-extrabold",
    weight: 800,
    style: "normal",
    filePath: "Montserrat-ExtraBold.ttf",
  },
];
...
async function loadFonts(origin: string) {
  return await Promise.all(
    FONTS.map(async (font) => {
      ...
      const url = [origin, "ogdata", filePath].join("/");
...
export async function generateOgImageResponse({
  ...
  console.log("Generating OG image for", type);
  try {
      ...
      (
        <div style={styles.container}>
          ...
          <div style={styles.author}>
            ...
            {type === "forum" && <div style={styles.divider}>&middot;</div>}
            {type === "forum" && (
              <span style={{ fontSize: 26, color: "#475569" }}>
                {replies.length}
              </span>
            )}
            {type === "forum" && <div style={styles.divider}>réponses</div>}
          </div>
          <div style={styles.borderBottom} />
        </div>
      ),
